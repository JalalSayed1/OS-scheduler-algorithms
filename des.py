...
class SchedulerDES(object):
    ...
    def __init__(self, *, num_processes, arrivals_per_time_unit, avg_cpu_burst_time, context_switch_time=0.0, quantum=math.inf):
        ...
        - quantum: max amount of time for which to execute the selected process (defaults to math.inf (infinity), i.e., batch mode)
        """
        ...
        self.time = 0
        self.events_queue = deque() # empty
        self.processes = list() # empty
    ...
    def __log_events_queue(self, level=logging.DEBUG):
        ...
        if not self._logger.isEnabledFor(level):
            return
        self.print_events_queue(partial(self._logger.log, level)) # partial is the printing function now
    ...
    def generate_and_init(self, seed=None): # seed will be send by the main function
        ...
        for i in range(self.num_processes):
            cur_time += self._arrival_time() # a random num
            new_process = Process(process_id=i, arrival_time=cur_time, service_time=self._service_time()) # service_time is a random num
            self.processes.append(new_process)
            new_event = Event(process_id=new_process.process_id, event_time=cur_time, event_type=EventTypes.PROC_ARRIVES)
            insort(self.events_queue, new_event) # sorted ascending order according to the arrival time
    ...
    def next_event_time(self):
        ...
        next_time = math.inf
        for event in self.events_queue:
            if event.event_time > self.time: # the queue is always sorted according to event time
    ...
    def run(self, seed=None):
        ...
        #* makes num_precesses elements in event_queue and precess_list
        #* event_queue is already sorted
